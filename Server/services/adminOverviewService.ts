...
const LAYER_META: Record<string, Omit<OverviewLayer, "phase" | "frontendPath" | "description">> = {
  ...
  market: {
    ...
    actionLabel: "Launch ->",
    note: "Commerce is now running a multi-vendor checkout stack, but live payout proof and launch inventory still need sovereign clearance.",
    checklist: [
      { item: "Stripe Connect configured", status: "done", required: true },
      { item: "Multi-vendor checkout + order confirm flow wired", status: "done", required: true },
      { item: "Vendor payout proof recorded from live transfer", status: "attention", required: true },
      { item: "Launch inventory seeded for day-one browsing", status: "attention", required: true },
    ],
  },
  work: {
    id: "work",
    ...
    actionLabel: "Manage",
    note: "CIRCUIT now has resilient matching and the hire-to-contract path is wired, but end-to-end escrow proof still gates launch.",
    checklist: [
      { item: "Freelancer profile flow live", status: "done", required: true },
      { item: "CIRCUIT recommendations return ranked matches", status: "done", required: true },
      { item: "Hire -> contract -> milestone creation flow wired", status: "done", required: true },
      { item: "Escrow fund -> release proof recorded", status: "attention", required: true },
    ],
  },
  mobile: {
    id: "mobile",
...
export async function buildForgeBriefingText(): Promise<string> {
  ...
  return [
    ...
    `${healthLine} Your action for today: record a live vendor payout proof and clear the final Market launch gate.`,
