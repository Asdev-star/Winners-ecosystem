...
const NICH_GROUPS = [
  {
    id: "african-tech",
    name: "African Tech & Startups",
    slug: "AfricanTech",
    emoji: "🖥️",
    description: "Founders, developers, investors - building the future",
    memberCount: 12450,
  },
  {
    id: "diaspora-business",
    name: "Diaspora In Business",
    slug: "DiasporaInBusiness",
    emoji: "✈️",
    description: "Africans abroad, returnees, global entrepreneurs",
    memberCount: 8920,
  },
  {
    id: "winners-creators",
    name: "Winners Creators",
    slug: "WinnersCreators",
    emoji: "🎨",
    description: "Designers, artists, musicians, content creators",
    memberCount: 6730,
  },
  {
    id: "afrobeats-marketing",
    name: "AfroBeat Marketing",
    slug: "AfroBeatMarketing",
    emoji: "🎵",
    description: "Music industry, brand deals, artist growth",
    memberCount: 5210,
  },
  {
    id: "finance-invest",
    ...
    slug: "FinanceInvest",
    emoji: "📈",
    description: "Trading, fintech, wealth building strategies",
    memberCount: 9840,
  },
  {
    id: "beauty-fashion",
    ...
    slug: "BeautyFashion",
    emoji: "💄",
    description: "Stylists, brands, influencers, fashion entrepreneurs",
    memberCount: 4150,
  },
];
    ...
    <div style={styles.groupCard} onClick={() => onSelect(group.slug)}>
      ...
      <div style={styles.groupCardHead}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {group.myRole && (
            <span
              style={{ ...styles.roleBadge, color: roleColor[group.myRole] }}
            >
              {group.myRole}
            </span>
          )}
...
function GroupDetail({ slug, onBack }: { slug: string; onBack: () => void }) {
  const [group, setGroup] = useState<(Group & { members?: any[] }) | null>(
    null,
  );
  ...
  if (error || !group)
    return (
      <div style={styles.errorState}>
        ...
        <button
          style={{ ...styles.joinBtn, marginTop: "16px" }}
          onClick={onBack}
        >
          ← Back
        </button>
          ...
          {(
            [
              ...
              ...(group.myRole === "OWNER" || group.myRole === "ADMIN"
                ...
                : []),
            ] as { id: "feed" | "members" | "admin"; label: string }[]
          ).map((t) => (
            <button
              key={t.id}
              style={{
              ...
              <div style={{ padding: "20px 24px" }}>
                ...
                >
                  ...
                  >
                    <button
                      style={
                        settingsSaving ? styles.postBtnDisabled : styles.postBtn
                      }
...
function NicheGroupCard({
  niche,
  onJoin,
}: {
  niche: (typeof NICH_GROUPS)[0];
  onJoin: (slug: string) => void;
}) {
  return (
...
export default function GroupsPage() {
  ...
  const handleJoin = async (slug: string) => {
    await fetch(`${API}/groups/${slug}/join`, {
      method: "POST",
      headers: authHeaders(),
    });
    loadGroups();
  ...
  const handleLeave = async (slug: string) => {
    ...
    await fetch(`${API}/groups/${slug}/leave`, {
      method: "POST",
      headers: authHeaders(),
    });
    loadGroups();
  ...
  const handleCreated = (g: Group) => {
    setGroups((prev) => [
      { ...g, memberCount: 1, postCount: 0, isMember: true, myRole: "OWNER" },
      ...prev,
    ]);
    setShowCreate(false);
    setSelected(g.slug);
          ...
          {platformLayers.map((p) => (
            <div key={p.name} style={styles.contextItem}>
              ...
              <span
                style={{
                  color:
                    p.name === "Community" ? "var(--gold)" : "var(--text-dim)",
                }}
              >
                {p.name}
              </span>
      ...
      <div style={styles.pageHeader}>
        <div>
          ...
          <p style={styles.pageDesc}>
            Join communities, share focused content, collaborate with your team.
          </p>
        </div>
        <button
      ...
      <div style={styles.filterRow}>
        <div style={styles.filterTabs}>
          {(
            ...
            ] as { id: "all" | "mine" | "discover"; label: string }[]
          ).map((f) => (
            <button
              key={f.id}
              style={{
