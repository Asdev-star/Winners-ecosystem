...
const css = `
  ...
  .bp-status-badge.trialing  { background: rgba(137,196,225,0.1); color: var(--ice);   border: 1px solid rgba(137,196,225,0.2); }
  .bp-status-badge.past-due  { background: rgba(248,113,113,0.1); color: var(--red);   border: 1px solid rgba(248,113,113,0.2); }

  .bp-portal-btn {
    background: transparent; border: 1px solid rgba(137,196,225,0.2); color: var(--ice);
  ...
  .bp-usage-card {
    ...
    border: 1px solid rgba(137,196,225,0.1); border-radius: 12px; padding: 16px 18px;
  ...
  .bp-plan {
    background: linear-gradient(135deg, #0f1923 0%, #0D1520 100%);
    border: 1px solid rgba(137,196,225,0.1); border-radius: 16px; padding: 24px;
  ...
  .bp-manage-info { }
  .bp-manage-title { font-size: 13px; font-weight: 700; color: var(--text); margin-bottom: 3px; }
  ...
  .bp-manage-spinner { width: 12px; height: 12px; border: 2px solid rgba(137,196,225,0.2); border-top-color: var(--ice); border-radius: 50%; animation: bp-spin 0.7s linear infinite; }

  .bp-danger {
    background: linear-gradient(135deg, #0f1923 0%, #0D1520 100%);
    border: 1px solid rgba(248,113,113,0.15); border-radius: 16px; padding: 20px 24px;
    display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;
  }
      ...
      <div className="bp-inner">
          ...
          <div className="bp-current-right">
            ...
            {isPaid && canManage && (
              <button className="bp-portal-btn" onClick={handlePortal} disabled={portalLoading}>
                {portalLoading
                  ? <><span className="bp-manage-spinner" /> Opening…</>
                  : <>↗ Manage Billing</>
                }
              </button>
            )}
        ...
        {isPaid && canManage && (
          <div className="bp-manage-row">
            <div className="bp-manage-info">
              <div className="bp-manage-title">Payment & Invoices</div>
              <div className="bp-manage-desc">Update payment method, download invoices, or manage your Stripe billing details.</div>
            </div>
            <button className="bp-manage-btn" onClick={handlePortal} disabled={portalLoading}>
              {portalLoading
                ? <><span className="bp-manage-spinner" /> Opening portal…</>
                : <>↗ Open Stripe Portal</>
              }
            </button>
          </div>
