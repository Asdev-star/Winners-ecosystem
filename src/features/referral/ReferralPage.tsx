...
const css = `
  ...
  /* Hero card */
  .rr-hero {
    background: linear-gradient(135deg, var(--surface) 0%, rgba(245,200,66,0.04) 100%);
  ...
  .rr-hero-glow { position: absolute; right: -40px; top: -40px; width: 200px; height: 200px; border-radius: 50%; background: radial-gradient(circle, rgba(245,200,66,0.08) 0%, transparent 70%); pointer-events: none; }
  ...
  .rr-hero-title { font-size: 28px; font-weight: 800; margin-bottom: 8px; }
  ...
  .rr-link-input {
    ...
    padding: 11px 14px; font-family: 'Space Mono', monospace; font-size: 12px;
    color: var(--text); outline: none;
  }
  .rr-copy-btn {
    background: var(--gold); color: #080B10; border: none; border-radius: 3px;
    padding: 11px 20px; font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 700;
    cursor: pointer; transition: opacity 0.15s; white-space: nowrap; flex-shrink: 0;
  }
  .rr-copy-btn:hover { opacity: 0.88; }
  .rr-copy-btn.copied { background: var(--green); color: #080B10; }
  ...
  .rr-section-header { padding: 16px 20px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
  .rr-section-title { font-size: 13px; font-weight: 700; }
  .rr-section-body { padding: 0; }
  ...
  .rr-item-avatar { width: 32px; height: 32px; border-radius: 50%; background: var(--gold-dim, rgba(245,200,66,0.12)); color: var(--gold); display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; flex-shrink: 0; }
  ...
  .rr-lb-rank { font-family: 'Space Mono', monospace; font-size: 11px; font-weight: 700; width: 24px; text-align: center; flex-shrink: 0; }
...
export default function ReferralPage() {
  ...
  const shareTwitter = () => {
    const text = encodeURIComponent(`Join Winners Ecosystem — the best revenue intelligence platform for serious businesses. Use my referral link to get started:`);
    window.open(`https://twitter.com/intent/tweet?text=${text}&url=${encodeURIComponent(data?.referralUrl ?? "")}`, "_blank");
  };

  const shareWhatsApp = () => {
    const text = encodeURIComponent(`Join Winners Ecosystem using my referral link: ${data?.referralUrl}`);
    window.open(`https://wa.me/?text=${text}`, "_blank");
  };

  const shareEmail = () => {
    const subject = encodeURIComponent("Join me on Winners Ecosystem");
    const body    = encodeURIComponent(`Hey,\n\nI've been using Winners Ecosystem to track my revenue and analytics. I think you'd love it.\n\nUse my referral link to sign up: ${data?.referralUrl}\n\nCheers`);
    window.open(`mailto:?subject=${subject}&body=${body}`);
  };

  if (loading) return <div style={{ padding: 40, fontFamily: "Space Mono, monospace", fontSize: 12, color: "var(--text-dim)" }}>Loading…</div>;
    ...
    <div className="rr-root">
      ...
      <div className="rr-hero">
        ...
        <div className="rr-link-box">
          <input className="rr-link-input" value={data?.referralUrl ?? ""} readOnly onClick={(e) => (e.target as HTMLInputElement).select()} />
          <button className={`rr-copy-btn${copied ? " copied" : ""}`} onClick={copy}>
            {copied ? "✓ Copied!" : "Copy Link"}
          </button>
        </div>
        <div className="rr-share">
          <button className="rr-share-btn" onClick={shareTwitter}>🐦 Share on Twitter</button>
          <button className="rr-share-btn" onClick={shareWhatsApp}>💬 Share on WhatsApp</button>
          <button className="rr-share-btn" onClick={shareEmail}>📧 Share via Email</button>
        </div>
      </div>
            ...
            {(!data?.leaderboard || data.leaderboard.length === 0) ? (
              ...
              data.leaderboard.map((entry: any) => (
                <div className={`rr-lb-item${entry.isCurrentUser ? " me" : ""}`} key={entry.rank}>
                  ...
                  <div className="rr-lb-count">{entry.referrals} referral{entry.referrals !== 1 ? "s" : ""}</div>
                </div>
              ))
            )}
          </div>
