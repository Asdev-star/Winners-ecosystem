# 🏆 WINNERS ECOSYSTEM — MASTER PROJECT STATE V4
### Single Source of Truth · Build Bible · Last Updated: February 28, 2026
### Upgraded: Design Strategy + AI Assistants Roadmap + Intelligence v2.0 Integrated
...
### Phase 1 — Niche Features to Build (from Design Strategy)

| Feature | Description | Priority |
|---|---|---|
| 🗺️ Journey Map | Visual map showing the user's path through all 8 layers | 🟡 Medium |
| 🏆 Achievement System | Cross-platform badges: 'Community Builder', 'Master Trader', 'Elite Freelancer' | 🟡 Medium |
| 📊 Wealth Dashboard | Real-time earnings across all layers in one view with charts | 🟡 Medium |
...
|---|---|---|---|
| 🖥️ African Tech & Startups | Founders, developers, investors | Building in public, startup updates, hiring | Academy tech courses → Work freelancers |
| 🎵 Music & Afrobeats | Artists, producers, managers | Music drops, behind-the-scenes, industry | Stream on Winners Market → sell beats |
...
- [ ] 📡 Live Spaces — Twitter-Spaces-style audio rooms, host + invite panellists, recorded + transcribed
...
- [ ] 📊 Creator Analytics — reach, impressions, follower growth, top posts, audience demographics
- [ ] 💎 Creator Economy — subscription tiers, exclusive posts, DMs, live sessions (15% platform cut)
...
- [ ] NOVA moderates content, flags edge cases for human review
...
| Paid Community Membership | Creators sell private group access with exclusive content | 15% platform commission |
| Sponsored Posts / Brand Deals | Connect brands with creators for sponsored content | 10% of deal value |
| Community Ads | Businesses advertise to specific community segments | CPM-based |
| Live Space Recording | Record spaces, add transcript, publish as Academy lesson | Creator keeps 70%, platform 30% |
| Diaspora Directory Premium | Enhanced business listing with logo, links, featured placement | $19/month |
...
- [ ] 📄 Lecture-to-Notes — upload lecture audio → SAGE generates structured notes automatically
- [ ] Assignment screenshot review — SAGE gives structured feedback
- [ ] 🎯 Skill-Based Paths — pre-built paths: African Fintech Developer, Digital Marketer, E-commerce Entrepreneur
- [ ] 🌍 Local Language Support — English, French, Swahili, Pidgin, Amharic (AI-assisted translation)
...
### What's Built (Demo Only — Needs Conversion to Production)
...
### V2.0 — ATLAS Goes Live

- [ ] ATLAS wired into `VendorDashboard.tsx` via `<AssistantPanel assistant="atlas" />`
- [ ] ATLAS analyses product photo → writes description + SEO tags automatically
- [ ] ATLAS reads supplier price list PDFs → builds comparison tables
- [ ] ATLAS reviews ad video submissions for platform policy compliance
- [ ] ATLAS reports revenue anomalies to OMEGA
...
### 🆕 Phase 5 V2.0 — Winners Intelligence Multimodal Platform (NEW)

This is the major upgrade. The existing single-provider Claude chat becomes a **full multimodal, multi-provider AI platform** with 9 named assistants.

#### Multimodal Architecture

```
USER UPLOADS FILE → React Frontend (image / PDF / audio / video)
         ↓  multipart/form-data  POST  ↓
WINNERS BACKEND → Node/Express  /api/v1/ai-platform/multimodal
         ↓  validate + auth + route  ↓
FILE PROCESSOR → FastAPI (validate · compress · base64 encode)
         ↓  provider routing  ↓
┌─ Claude API ─── images · PDFs · text (best quality + reasoning)
├─ GPT-4o ─────── images · audio · video · text (best audio)
├─ Gemini ──────── images · video · audio · PDFs (best video, native)
└─ Ollama (local) ─ text · images (LLaVA) — offline · free · private
```

#### Provider Routing Rules

| File Type | Formats | Primary Provider | Fallback | Notes |
|---|---|---|---|---|
| Images | JPEG · PNG · GIF · WebP | Claude 3.5 Sonnet | GPT-4o | Best reasoning about visual content |
| PDF Documents | PDF (any version) | Claude 3.5 Sonnet | GPT-4o (text extract) | Claude has native PDF support |
| Audio | MP3 · WAV · M4A · OGG | GPT-4o Whisper | Gemini Audio | Best transcription accuracy |
| Video | MP4 · WebM · MOV | Gemini 1.5 Pro | GPT-4o (frame extract) | Only Gemini supports video natively |
| Text only | Any | Ollama (local) | Claude / GPT-4o | Free — no API cost for text chat |

#### New Files to Build — AI Platform Service (Python FastAPI on port 8001)

```
ai-platform/
├── main.py
├── routers/
│   ├── multimodal.py          # POST /api/multimodal/chat
│   └── assistants.py          # POST /assistants/{name}/chat
├── services/
│   ├── file_processor.py      # validate · compress · base64 encode
│   ├── provider_router.py     # smart model selection per modality
│   ├── assistant_context.py   # 9 supervisor system prompts
│   ├── claude_client.py       # images + PDFs + text
│   ├── openai_client.py       # audio + images (Whisper)
│   ├── gemini_client.py       # video + audio + PDFs
│   ├── ollama_client.py       # local text + images (LLaVA)
│   └── file_storage.py        # Cloudinary for conversation history
```

#### New Files to Build — Node/Express Backend

```typescript
Server/routes/aiPlatformRoutes.ts    # proxy routes to FastAPI + multer file upload
  // POST /api/v1/ai-platform/multimodal
  // POST /api/v1/assistants/:name/chat
```

#### New Files to Build — Frontend

```typescript
src/hooks/useMultimodalChat.ts         # FormData POST with file attachments
src/hooks/useAssistant.ts              # named assistant hook (NOVA, SAGE, ATLAS etc.)
src/components/ai/AssistantPanel.tsx   # reusable panel, embeds in all 8 layers
src/components/ai/FileDropZone.tsx     # drag-and-drop: images, PDFs, audio, video
src/components/ai/ModelSelector.tsx    # toggle: Claude / GPT-4o / Gemini / Ollama
src/features/intelligence/omega/OmegaDashboard.tsx  # OMEGA supervision interface
```

#### Prisma Schema — New Models Needed
...
#### AI Credits — Monetisation Model

| Action | Credits | Approx Cost | Provider |
|---|---|---|---|
| Text message to any assistant | 2 credits | ~$0.002 | Ollama (free) / Claude |
| Image analysis | 5 credits | ~$0.01 | Claude / GPT-4o |
| PDF analysis — per document | 6 credits | ~$0.012 | Claude (native) |
| Audio transcription — per minute | 4 credits | ~$0.006 | Whisper / GPT-4o |
| Video analysis — per minute | 10 credits | ~$0.025 | Gemini 1.5 Pro |
| OMEGA autonomous action | 15 credits | ~$0.03 | Claude (high reasoning) |
| OMEGA daily ecosystem briefing | 50 credits | ~$0.10 | Claude + PDF analysis |
| Intelligence Pro (subscription) | Unlimited | $19/month | All providers |

#### Intelligence Plans

| Plan | Price | Credits/month | File Types | Providers |
|---|---|---|---|---|
| Free | Included | 100 credits | Images only | Ollama local |
...
| Intelligence Team | $49/month | 10,000 shared | All modalities | All + priority |
| Desktop License | $49 one-time | Unlimited local | All offline | Ollama + Whisper |
| OMEGA Enterprise | $500–$5K/month | Unlimited | All + autonomous | Dedicated infra |
| White-label Assistants | Custom | — | All | Custom |
...
- [ ] 🏅 Academy-Linked Profiles — certificates appear as verified skill badges on freelancer profiles
- [ ] 🤖 CIRCUIT AI Matching — reads job description + freelancer profiles, generates match scores
- [ ] 📝 AI Proposal Generator — CIRCUIT writes custom proposals based on freelancer profile + job
...
|---|---|---|---|
| Software Development | React, Node.js, Python, mobile | $500–$10,000/project | High diaspora demand, global remote clients |
| Digital Marketing | Social media, paid ads, SEO | $200–$3,000/month | African brands scaling globally |
| Creative & Design | UI/UX, graphic design, video | $300–$5,000/project | Brand identity for African startups |
| Writing & Content | Copywriting, blogging, translation | $50–$500/article | African language localisation |
| Financial Services | Bookkeeping, financial models | $100–$500/hour | Diaspora tax, African startup CFO |
| Business Consulting | Strategy, market entry, pitch decks | $500–$5,000/engagement | Pan-African expansion advice |
| Music & Entertainment | Production, mixing, management | $200–$10,000/project | Afrobeats global expansion |
...
**Key Mobile Features:**
- 🎙️ Voice-First Interface — tap to talk to any assistant, voice posts, voice-to-text
- 📸 Camera Integration — photograph a product for ATLAS, photograph assignment for SAGE
- 💳 Mobile Payments — M-Pesa, Airtel Money, Orange Money, MTN MoMo, Flutterwave
- 📥 Offline Courses — download up to 10 Academy lessons, progress syncs when online
- 🔔 Smart Notifications — AI-curated push (OMEGA decides what's relevant)
...
|---|---|---|
| 🤖 AI Assistant API | Call any of the 9 assistants via REST API. Multimodal support | Pay per token |
| ✅ Identity Verification API | Verify Trust Score, Academy certs, Work history | $0.50 per verification / $49/month |
| 💳 Payments API | Stripe + Flutterwave in one integration, M-Pesa, MTN MoMo | Transaction-based |
| 🎓 Certificate Verification API | Verify Academy certificates in real-time | $0.50 per verification |
| 🔌 Plugin Marketplace | Publish plugins that extend any of the 9 assistants | 70% developer / 30% platform |

### SDK Packages

| Language | Package | Status |
|---|---|---|
| JavaScript / TypeScript | `@winners/sdk` | Build first |
| Python | `winners-py` | Build with JS |
| Go | `winners-go` | Phase 2 |
| Swift / Kotlin | `winners-mobile` | With mobile app |
...
AI (existing):  Anthropic Claude API (Aria chatbot + AI recommendations)
AI Platform:    Python FastAPI (port 8001 — sidecar service) [NEW]
AI Providers:   Claude · GPT-4o · Gemini · Ollama (local) [NEW]
...
src/features/intelligence/omega/OmegaDashboard.tsx    📋  NOT built
src/components/ai/AssistantPanel.tsx                  📋  NOT built — needed for all layers
src/components/ai/FileDropZone.tsx                    📋  NOT built
src/components/ai/ModelSelector.tsx                   📋  NOT built
src/hooks/useMultimodalChat.ts                        📋  NOT built
src/hooks/useAssistant.ts                             📋  NOT built
...
### 🔴 Add New Railway Environment Variables

```env
ANTHROPIC_API_KEY=sk-ant-...
OPENAI_API_KEY=sk-...
GOOGLE_API_KEY=AIza...
AI_PLATFORM_URL=http://ai-platform:8001
...
- [ ] Build AI Platform Python FastAPI service (Phase 0 of multimodal roadmap)
- [ ] `npm install multer @types/multer` in Server/
- [ ] Build `aiPlatformRoutes.ts` — proxy to FastAPI
...
- [ ] `npx prisma migrate dev --name add_assistant_infrastructure`
...
| 🟢 9 | Build OMEGA Dashboard at /intelligence/omega | Intelligence | Ecosystem supervision goes live |
| 🟢 10 | PWA setup: service worker + manifest + mobile-responsive all pages | Mobile | Installable on mobile home screen — Africa-first |
...
- An **AI infrastructure** every layer depends on (Intelligence) orchestrated by OMEGA
...
*Last updated: February 28, 2026 · Version 4.0 · winners-empire-eco.up.railway.app*
*Incorporates: Design Strategy v1.0 + AI Assistants Roadmap + Intelligence v2.0 Build Spec + Repo State Analysis*
